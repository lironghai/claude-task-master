/**
 * src/ai-providers/Qwen-cli.js
 *
 * Implementation for interacting with Qwen models via Qwen CLI
 * using the ai-sdk-provider-Qwen-cli package.
 */
...
let createQwenProvider;

async function loadQwenCliModule() {
	if (!createQwenProvider) {
		try {
			const mod = await import('ai-sdk-provider-Qwen-cli');
			createQwenProvider = mod.createQwenProvider;
		} catch (err) {
			throw new Error(
				"Qwen CLI SDK is not installed. Please install 'ai-sdk-provider-Qwen-cli' to use the Qwen-cli provider."
			);
		}
	}
}

export class QwenCliProvider extends BaseAIProvider {
	constructor() {
		super();
		this.name = 'Qwen CLI';
	}
	...
	/**
	 * Override validateAuth to handle Qwen CLI authentication options
	 * @param {object} params - Parameters to validate
	 */
	validateAuth(params) {
		// Qwen CLI is designed to use pre-configured OAuth authentication
		// Users choose Qwen-cli specifically to leverage their existing
		// Qwen auth login credentials, not to use API keys.
	...
	/**
	 * Creates and returns a Qwen CLI client instance.
	 * @param {object} params - Parameters for client initialization
	 * @param {string} [params.apiKey] - Optional Qwen API key (rarely used with Qwen-cli)
	 * @param {string} [params.baseURL] - Optional custom API endpoint
	 * @returns {Promise<Function>} Qwen CLI client function
	 * @throws {Error} If initialization fails
	 */
	async getClient(params) {
		try {
			// Load the Qwen CLI module dynamically
			await loadQwenCliModule();
			// Primary use case: Use existing Qwen CLI authentication
			// Secondary use case: Direct API key (for compatibility)
			let authOptions = {};

			if (params.apiKey && params.apiKey !== 'Qwen-cli-no-key-required') {
				...
				// Expected case: Use Qwen CLI authentication via OAuth
				authOptions = {
			...
			return createQwenProvider(authOptions);
		} catch (error) {
	 ...
	 * This is needed because ai-sdk-provider-Qwen-cli expects system prompts as a separate parameter.
	...
	_extractSystemMessage(messages, options = {}) {
		...
		// Add Qwen CLI specific JSON enforcement if requested
	...
	/**
	 * Gets a Qwen CLI specific system prompt to enforce strict JSON output
	...
	/**
	 * Simplifies complex prompts for Qwen-cli to improve JSON output compliance
	 * @param {Array} messages - Array of message objects
...
Generate ${subtaskCount} subtasks based on the original task context. Return ONLY the JSON object.`;
	...
	/**
	 * Extract JSON from Qwen's response using a tolerant parser.
	 *
	...
	/**
	 * Generates text using Qwen CLI model
	...
	async generateText(params) {
		try {
			...
			// Detect if JSON output is expected and enforce it for better Qwen-cli compatibility
			...
			// For Qwen-cli, simplify complex prompts before processing
			...
			// If we detected a JSON request and Qwen-cli returned conversational text,
			...
			if (enforceJsonOutput && result.text && !this._isValidJson(result.text)) {
				...
				// Log first 1000 chars of the response to see what Qwen actually returned
	...
	/**
	 * Streams text using Qwen CLI model
	 * Overrides base implementation to properly handle system messages and enforce JSON output when needed
	 */
	...
	/**
	 * Generates a structured object using Qwen CLI model
	 * Overrides base implementation to handle Qwen-specific JSON formatting issues and system messages
	 */
	async generateObject(params) {
		try {
			...
			if (error.message?.includes('JSON') || error.message?.includes('parse')) {
				log(
					'debug',
					`Qwen CLI generateObject failed with parsing error, attempting manual extraction`
				);
				...
				try {
					...
					const result = await generateObject({
						...
						mode: 'json', // Use json mode instead of auto for Qwen
						maxTokens: params.maxTokens,
						temperature: params.temperature
					});
					...
					if (result.rawResponse?.text && !result.object) {
						...
						try {
							...
							throw new Error(
								`Qwen CLI returned invalid JSON that could not be parsed: ${parseError.message}`
							);
						}
					}
					...
					log(
						'error',
						`Qwen CLI manual JSON extraction failed: ${retryError.message}`
					);
